...
class Person(pygame.sprite.Sprite):
    def __init__(self, x, y, nome, dano_normal, dano_especial, vida_max, ki_max, controle, is_facing_right=True):
        ...
        self.cooldown_ataque_especial = 1000  # Cooldown de 1 segundo
    ...
    def update(self, keys, tempo_decorrido, tela):
        ...
        if self.controle == 'wasd':
            ...
            # Ataque normal
            if keys[pygame.K_j]:
                if not self.is_attacking and not self.ataque_especial_ativo:
                    self.estado = 'ataque'
                    self.is_attacking = True
                    self.tempo_ataque = tempo_decorrido

# Ataque especial — tem prioridade e só ocorre se não houver ataque ativo
            if keys[pygame.K_k]:
